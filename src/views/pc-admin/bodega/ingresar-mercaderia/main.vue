...
<template>
  <div>
    ...
    <div class="subtitle-1 mt-3 mb-3 text--secondary">
      Aqui Podras Crear, Actualizar y Eliminar {{ ModuloFullName }}.
    </div>
    <v-divider></v-divider>
    <v-row>
      ...
      <v-tab-item v-for="i in 3" :key="i" :value="'mobile-tabs-5-' + i">
        <v-card flat>
          <v-card-text v-if="i == 1">
            ...
            <v-simple-table v-if="Datos.length > 0 && Loader == false">
              <template v-slot:default>
                <thead>
                  <tr>
                    <th class="text-left">Ingreso/Salida</th>
                ...
                <tbody>
                  <tr v-for="Dato in Datos" :key="Dato._id">
                    <td style="width: 6rem">
                      {{
                        Dato.createdAt
                          .slice(0, 10)
                    ...
                    <td v-else style="width: 9rem">
                      <div
                        class="row"
                        v-for="(Ubicacion, i) in Dato.Producto.Ubicacion"
                        :key="i"
                        style="margin: 0"
                      >
                        <v-col
                          ...
                          :style="
                            i == 0
                              ? 'padding: 0;'
                              : 'padding: 0.2rem;border-top: 1px solid lightgray;'
                          "
                          >{{ Ubicacion.Bodega }} 10{{ Ubicacion.Fila }}00{{
                            Ubicacion.Columna
                          }}00{{ Ubicacion.Nivel }}</v-col
                        >
                      </div>
                    </td>
          ...
          <v-card-text v-if="i == 2">
            <!-- Ultimas Entradas  -->
            <v-simple-table v-if="Entradas.length > 0 && Loader == false">
              <template v-slot:default>
                <thead>
                  <tr>
                    <th class="text-left">Ingreso/Salida</th>
                ...
                <tbody>
                  <tr v-for="Dato in Entradas" :key="Dato._id">
                    <td style="width: 6rem">
                      {{
                        Dato.createdAt
                          .slice(0, 10)
            ...
            <v-simple-table v-else-if="Entradas.length == 0 && Loader == false">
              <template v-slot:default>
                <thead>
                  <tr>
                    <th class="text-left">
          ...
          <v-card-text v-if="i == 3">
            <!-- Ultimas Salidas  -->
            <v-simple-table v-if="Salidas.length > 0 && Loader == false">
              <template v-slot:default>
                <thead>
                  <tr>
                    <th class="text-left">Ingreso/Salida</th>
                ...
                <tbody>
                  <tr v-for="Dato in Salidas" :key="Dato._id">
                    <td style="width: 6rem">
                      {{
                        Dato.createdAt
                          .slice(0, 10)
            ...
            <v-simple-table v-else-if="Salidas.length == 0 && Loader == false">
              <template v-slot:default>
                <thead>
                  <tr>
                    <th class="text-left">
        ...
        <template>
          <v-card>
            <v-card-text class="text-center">
              <v-alert shaped dense dark color="grey">
                Crear {{ ModuloName }}
              </v-alert>
                ...
                <v-col cols="2">
                  <v-container class="px-0" fluid>
                    <v-switch
                      ...
                      @click="
                        UpdateIva(
                          ProveedorSeleccionado.rut,
                          ProveedorSeleccionado.iva
                        )
                      "
                    >
              ...
              <v-col cols="12">
                <div class="center" v-if="LoaderFactura == true">
                  <div
                    class="spinner"
                    style="max-width: 1rem; margin: 1rem"
                  ></div>
              ...
              <v-row
                ...
                "
              >
                <v-alert class="mt-2" dense outlined type="error">
                  <strong
                    >No se encontro la factura en el Servicio de Impuestos
                    Internos. verifique el numero de folio y proveedor.</strong
                  >
                </v-alert>
                <v-col cols="12" class="center">
                  <v-btn color="success" @click="ApiGetFactura = true"
                    >Ingresar de todos modos.</v-btn
                  >
                </v-col>
              </v-row>
                        ...
                        >
                          ...
                          <td>
                            <v-text-field
                              ...
                              @blur="
                                BuscarProducto(i, Producto.CodigoImportadora)
                              "
                            >
                            </v-text-field>
                          </td>
                          ...
                          <td>
                            <v-btn
                              v-if="ProductosRegistrados.length != 1"
                              color="error"
                          ...
                          <template v-slot:default>
                            <tbody>
                              <tr>
                                ...
                                >
                                  {{
                                    FormatPrecio(Math.round(PrecioTotal / 1.19))
                                  }}
                                  $
                                </td>
                              </tr>
                              ...
                              <tr>
                                ...
                                <td
                                  ...
                                  style="text-align: end"
                                >
                                  {{ FormatPrecio(Math.round(PrecioTotal)) }} $
                                </td>
                              </tr>
                            </tbody>
                          </template>
            ...
            <v-card-actions class="justify-end" v-if="Loader == false">
              ...
              <v-btn text color="primary" @click="Crear()" v-else
                >Crear {{ ModuloName }}</v-btn
              >
              <v-btn text color="secondary" @click="dialogCrear = false"
                >Cerrar</v-btn
              >
            </v-card-actions>
          </v-card>
      ...
      >
        <template>
          <v-card>
              ...
              <div v-if="Loader == false">
                      ...
                      <template v-slot:default>
                        ...
                        <tbody>
                          <tr>
                            <td>
                              {{
                                ProductoSeleccionado.createdAt
                                  .slice(0, 10)
                                  .split("-")
                                  .reverse()
                                  .join("-")
                            ...
                            <td>
                              <v-text-field
                                ...
                                :disabled="
                                  CantidadAnterior !=
                                  ProductoSeleccionado.CantidadRestante
                                    ? true
                                    : false
                                "
                              >
                ...
                <v-simple-table>
                  <template v-slot:default>
                    ...
                    <tbody>
                      <tr v-for="(Ubicacion, i) in Ubicaciones" :key="i">
                        <td>
                          <v-select
                            class="mt-5"
                        ...
                        <td>
                          <v-select
                            ...
                            dense
                            :disabled="
                              Ubicacion.Filas.length == 0 ? true : false
                            "
                          ></v-select>
                        </td>
                        <td>
                          <v-select
                            ...
                            dense
                            :disabled="
                              Ubicacion.Columnas.length == 0 ? true : false
                            "
                          ></v-select>
                        </td>
                        <td>
                          <v-select
                            ...
                            dense
                            :disabled="
                              Ubicacion.Niveles.length == 0 ? true : false
                            "
                          ></v-select>
                        </td>
                        <td v-if="i == 0">
            ...
            <v-card-actions class="justify-end" v-if="Loader == false">
              <v-btn text color="primary" @click="UpdateRegistro()"
                >Actualizar</v-btn
              >
...
export default {
  ...
  methods: {
    async UpdateRegistro() {
      ...
      if (this.$refs.formUbicacion.validate()) {
        ...
        if (
          ...
          this.ProductoSeleccionado.PrecioImportadora < 1000 ||
          this.ProductoSeleccionado.Cantidad == 0
        ) {
          return (this.Alert =
            "ERROR: VERIFIQUE QUE LOS CAMPOS ESTEN CORRECTOS");
        } else {
          this.Loader = true;
          this.Datos = [];
          // Preparar el Json
          let Json = {
            ...
            PrecioImportadora: parseInt(
              this.ProductoSeleccionado.PrecioImportadora.toString()
                .replaceAll(".", "")
                .replaceAll(",", "")
            ),
    ...
    async EditarUbicacion(Producto) {
      ...
      this.Ubicaciones = Producto.Producto.Ubicacion;
      ...
      this.ProductosRegistrados.filter((e, a) => {
        if (i == a) {
          if (e.Cantidad < 1) {
            ...
            e.Cantidad = parseInt(
              e.Cantidad.toString().replace(/\./g, "").replace(/\,/g, "")
            );
      ...
      this.ProductosRegistrados.filter((e, a) => {
        ...
        if (i == a) {
          if (e.PrecioImportadora < 100) {
            ...
            e.PrecioImportadora = parseInt(
              e.PrecioImportadora.toString()
                .replace(/\./g, "")
                .replace(/\,/g, "")
            );
    ...
    async BuscarFactura() {
      if (this.ProveedorSeleccionado == "" || this.Folio == "") {
        ...
        let Factura = await API.POST_DTE_COMPRA(
          this.ProveedorSeleccionado.rut,
          this.Folio
        );
        ...
        if (!Factura.detalle) {
          this.Alert =
            "SE ENCONTRO LA FACTURA PERO LOS DETALLES SE DEBEN INGRESAR MANUALMENTE, TOTAL: " +
            Factura.total;
          this.AlertType = "info";
        ...
        this.ProductosRegistrados.filter(async (e, a) => {
          ...
          if (i == a && e.CodigoImportadora != null) {
            ...
            if (Product[0]) {
              ...
              this.PrecioController();
              if (
                this.ProductosRegistrados[this.ProductosRegistrados.length - 1]
                  .Descripcion == null ||
                this.ProductosRegistrados[this.ProductosRegistrados.length - 1]
                  .Descripcion == ""
              ) {
    ...
    async GetDatos() {
      ...
      Res = Res.map((e) => {
        if (e.Salida == true) {
          e.Folio = e?.Boleta?.Folio || e?.Factura?.Folio || 0,
            e.razon_social = e?.Boleta?.Nombres? e?.Boleta?.Nombres + " " + e?.Boleta?.Apellidos : e?.Factura?.RazonSocialEmpresa ?  e?.Factura?.RazonSocialEmpresa : "N/A";
          return e;
        } else {
          return e;
        }
      ...
      this.Entradas = Res.filter((e) => {
        if (e.Entrada == true) {
          return e;
        }
      });

      this.Salidas = Res.filter((e) => {
        if (e.Salida == true) {
          return e;
        }
      });
    ...
    async GetDato(Id) {
      ...
      this.AlertType = "error";
      (this.Alert = ""), this.$refs.formCrear.resetValidation();
