        ...
        >
            <template v-slot:item="row">
                <tr>
                  ...
                  <td>{{row.item.Importadora}} {{ row.item.CantidadAgregada}} {{ spinnerCantidad}}</td>
                  ...
                  <!-- <td>
                    ...
                    <td style="width: 7rem">
                      <v-btn class="mx-2" fab dark small color="blue" v-if="row.item.CantidadAgregada == 0"  @click="AgregarCarritoEmit(row.item.CodigoImportadora, 1, row.item.Modelo)">
...
Productos Nuevos originales y alternativos.`;
    ...
    async modelSearch() {
      ...
      //Productos
      Datos.ProductosList = Datos.ProductosList.map(e => {
      ...
      this.ProductosCount = Datos.ProductosList.lenght + 1;


      var e = Datos.ProductosList.lenght;
        for(let i = 0; i < e; i++){
